...
def test_bima_form_manual_checkbox():
    with sync_playwright() as playwright:
        try:
            ...
            page.goto("https://test.online.bima.tj/", timeout=60000, wait_until="domcontentloaded")

            # Навигация к разделу "Коробочные продукты"
            print("Открытие меню навигации...")
            page.get_by_role("navigation").get_by_role("button").click()
            print("Переход в 'Коробочные продукты'...")
            page.get_by_role("link", name="Коробочные продукты").click()

            # Нажатие на кнопку "Купить" для конкретного товара
            print("Нажатие на кнопку 'Купить'...")
            page.locator("div:nth-child(15) > .main-popular__item > .slider-products__info > .slider-products__links > a").first.click()
