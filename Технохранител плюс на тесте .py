...
def run(playwright: Playwright) -> None:
    ...
    page.locator("div:nth-child(6) > .main-popular__item > .main-popular__info > .main-popular__links > a").first.click()
    ...
    page.get_by_placeholder("Отчество").fill("Test")
    page.get_by_text("Мужской").first.click()
    page.get_by_text("Мужской").nth(1).click()
    page.get_by_label("Дата рождения").fill("2006-12-21")
    page.get_by_text("Таджикистан").first.click()
    page.get_by_text("Албания").click()
    page.get_by_placeholder("00 00 00").click()
    page.get_by_placeholder("00 00 00").fill("987 98 78 979")
    ...
    page.get_by_placeholder("Кем выдан паспорт").press("Tab")
    page.get_by_label("Дата выдачи").fill("2023-12-12")
    page.locator("div:nth-child(4) > .policy__wrapper > div > label > .label__input > .light-select").click()
    page.get_by_text("ГБАО").click()
    ...
    page.get_by_text("3D очки").click()
    page.locator(".column__wrapper-3 > .label > .label__input > .light-select").click()
    page.get_by_text("Asus").click()
    page.get_by_placeholder(" Модель").click()
    page.get_by_placeholder(" Модель").fill("1")
    page.get_by_placeholder(" IMEI/Серийный номер").click()
    page.get_by_placeholder(" IMEI/Серийный номер").fill("Test65757GFH")
    page.get_by_label("Дата покупки").fill("2024-12-03")
    page.get_by_placeholder(" Стоимость").click()
    page.get_by_placeholder(" Стоимость").fill("837826")
